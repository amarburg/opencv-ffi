...
module CVFFI
  ...
  class CvMat
    ...
    def resize( size, interpolation = :CV_INTER_LINEAR )
      ...
      dest = CvMat.create( sz,  {type: type} )
    ...
    def *(b)
      ...
      when Mat, CvMat, Matrix
        ...
        raise "Other array is not same type as self (%d,%d)" % [ self.depth, b.depth ] unless self.type == b.type
    ...
    def ensure_greyscale
      ...
      CVFFI::cvCvtColor( self, greyImg, :CV_BGR2GRAY )
      greyImg
    end
    ...
    class << self
      ...
      def eye( x, opts = {} )
        type = opts[:type] || :CV_32F
        a = CvMat.create( x,x, type )
        CVFFI::cvSetIdentity( a, CVFFI::cvScalarAll( 1 ) )
        a
      end

      def build( rows, cols, opts = {}, &blk )
        CvMat.create( rows, cols, opts ) { |i,j| blk.call(i,j) }
      end

      def rows( row_vector, opts = {} )
      ...
      def columns( col_vector, opts = {} )
        cols = col_vector.length
        rows = col_vector.first.length
        col_vector.each { |col| raise "Not all columns in vector are same length." unless col.length == rows }
